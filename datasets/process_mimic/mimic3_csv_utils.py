...
Contains utils methods for preprocessing MIMIC data
'''
...
# returns a pandas df from a csv path, defaults to header = 0, index_col = 0
...
def read_patients_table(path):
    '''
    returns a pandas df with patient info: id, gender, age, date of death, etc., core/patients.csv
    '''
    patients_df = dataframe_from_csv(path)
    columns = ['subject_id', 'gender', 'anchor_age', 'dod']  
    patients_df = patients_df[columns]
...
def read_admissions_table(path):
    '''
    returns pandas df with the hospital admissions data, core/admissions.csv
    '''
...
def read_icustays_table(path):
    '''
    returns a pandas df with info about patient icu stays, icu/icustays.csv
    '''

    stays = dataframe_from_csv(path)
...
def remove_icustays_with_transfers(stays):
    '''
    make sure the unit stays the same throughout the patient visit
    '''
...
def merge_on_subject_admission(table1, table2):
    '''
    we want to link the admission and icu stay from the same patient
    '''
...
def merge_on_subject(table1, table2):
    '''
    link patient info to the admission + icu stay info
    '''
...
def filter_admissions_on_nb_icustays(stays, min_nb_stays=1, max_nb_stays=1):
    '''
    keep only the patients who have 1 admission (can be toggled)
    '''
...
def add_age_to_icustays(stays):
    '''
    adds age col that uses the 'anchor_age' col
    '''
...
def add_inunit_mortality_to_icustays(stays):
    '''
    'mortality_inunit' as a binary int col that represents patient mortality based on the 'dod' and/or 'deathtime' col (USING intime and outtime)
    '''
...
def add_inhospital_mortality_to_icustays(stays):
    '''
    'mortality_inhospital' as a binary int col that represents patient mortality based on the 'dod' and/or 'deathtime' col (USING admittime and dischtime)
    '''
...
def filter_icustays_on_age(stays, min_age=18, max_age=np.inf):
    '''
    remove patients that are less than 18 years old
    '''
...
def read_icd_diagnoses_table(path):
    '''
    adds human readable 'long_title' annotations to the diagnoses given to patients, 'icd_code'
    '''
...
def filter_diagnoses_on_stays(diagnoses, stays):
    '''
    inner join with icu stay and diagnoses codes for the 'subject_id' and 'hadm_id'
    '''
...
def count_icd_codes(diagnoses, output_path=None):
    '''
    count and sort the number of stays that each icd_code was used in, save this to a csv in DESC order
    '''
...
def break_up_stays_by_subject(stays, output_path, subjects=None):
    '''
    creates icu stay data into files for each individual subject
    '''
...
def break_up_diagnoses_by_subject(diagnoses, output_path, subjects=None):
    '''
    break up the diagnoses data by subject into their own individual files
    '''
